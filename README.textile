...
The idea of this module is to provide automatic buffering for I/O on Linux at the level of system calls. Files are being watched while they are read, and whenever a  large file is read in many small chunks (which is the default behaviour of most programs), a buffer of up to 4 MB is allocated in kernel memory, and file content is pre-fetched.
